...
# from data.geometric_augmentations import apply_affine_transform_to_trajectory
...
def confidence_stat(batch, output):
    dist = torch.sum((output['tracks'] - batch['target_points']) ** 2, dim=-1)
    visible = (batch['occluded'] == 0)

    metrics = {}

    conf_thres = (1, 2, 4, 8, 16)
    for thres in conf_thres:
        conf_gt = (dist < thres ** 2) & (visible)
        # conf_pred = (output['initial_stage_confidence'] > 0.3) & (torch.sigmoid(output['expected_dist']) < 0.5)

        # conf_pred = (torch.sigmoid(output['expected_dist']) < 0.5) & (torch.sigmoid(output['occlusion']) < 0.5)

        # conf_pred = (torch.sigmoid(output['occlusion']) < 0.5) & (output['initial_stage_confidence'] > 0.3)
        # conf_pred = (torch.sigmoid(output['occlusion']) < 0.5) & (output['initial_stage_confidence'] > 0.8)
        # conf_pred = (torch.sigmoid(output['occlusion']) < 0.45)
        # cut 80% of the lowest confidence
        conf_pred = output['initial_stage_confidence'] > torch.quantile(output['initial_stage_confidence'], 0.8)
        conf_pred = conf_pred & (torch.sigmoid(output['occlusion']) < 0.5)

        tp = torch.sum(conf_gt & conf_pred)
        fp = torch.sum(~conf_gt & conf_pred)
        fn = torch.sum(conf_gt & ~conf_pred)
        tn = torch.sum(~conf_gt & ~conf_pred)
        f1 = 2 * tp / (2 * tp + fp + fn)
        precision = tp / (tp + fp)
        recall = tp / (tp + fn)
        accuracy = (tp + tn) / (tp + tn + fp + fn)

        print(f'Confidence threshold: {thres}')
        print(f'F1: {f1}')
        print(f'Precision: {precision}')
        print(f'Recall: {recall}')
        print('---')
        metrics[f'f1_{thres}'] = f1.item()
        metrics[f'precision_{thres}'] = precision.item()
        metrics[f'recall_{thres}'] = recall.item()
        metrics[f'accuracy_{thres}'] = accuracy.item()

    survive_ratio = conf_pred[visible].float().mean()
    metrics['survive_ratio'] = survive_ratio.item()
    print(f'Survive Ratio: {survive_ratio * 100:.2f}%')
